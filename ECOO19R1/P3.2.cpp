...
bool clear_side_back(int si, int sj){
	for(int j=sj;j>=sj-2;j--){		//sideways two
		if(g[si][j]=='@'){
			//printf("  side %d %d\n", si, j);
			return false;
...
bool clear_up_back(int si, int sj){
	bool ret=false;
	//printf(":%d\n", si-l);
	for(int i=si;i>=si-l;i--){		//jumping up
		if(g[i][sj]!='@'){
			//printf(" %d %d\n", i, sj);
			ret=ret||clear_side_back(i, sj);
		}
	}
	return ret;
}
	...
	for(int test=1;test<=10;test++){
		...
		if(!suc){		//try running backwards?
			...
			while(!q.empty()){
				...
				r[cj]=true;
				//printf(" %d %d\n", ci, cj);
				if(g[ci][cj-1]!='@'&&!inq[ci][cj-1]){		//can we run BACKWARDS?
					q.push({ci, cj-1});
					inq[ci][cj-1]=true;
				}
				if(ci==h-1&&clear_up_back(ci, cj)&&!inq[ci][cj]){		//can we jump BACKWARDS?
					q.push({ci, cj-2});
					inq[ci][cj]=true;
				}
			}
